...
//UPDATE MR.108P
  ...
  if (args.length < 1) return reply(`Contoh: ${prefix}crossfirelogo ArnandoGanz`)
    cros = body.slice(14)
    reply(ind.wait())
bit = await getBuffer(`https://api.vhtear.com/hacker_avatar?text=${cros}&apikey=BeliApikey`)
...
case 'waifukawai'://UPDATE MR.108P
		...
		case 'kuni'://UPDATE MR.108P
				...
				reply(ind.wait())	
						...
						MrG.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
					} catch (e) {
				console.log(`Error :`, color(e,'red'))
				reply('тЭМ *ERROR* тЭМ')
					}
